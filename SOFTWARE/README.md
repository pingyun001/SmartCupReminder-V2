...
- SIGNATURE_TOOLS：应用程序签名工具，可为新编译完成的应用程序签名，以便bootloader识别并自动更新
...
2. bootloader中尝试挂载文件系统，如果成功，则尝试读取更新固件
    ...
    - 如果上述任意一步出错，则重建当前文件系统，尝试挂载成功后，开启USB并进入U盘模式，等待用户更新固件
3. 运行应用程序固件，启动wifi连接，连接成功后，尝试连接天气API，获取天气信息，并显示在屏幕上

# 固件更新说明
具体流程推荐详细阅读RELEASE_FIRMWARE文件夹中的README.md文件
