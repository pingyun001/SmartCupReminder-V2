                                      ...
                                      uint32_t totsize, uint32_t cpdsize, uint8_t fwmode)
{
...
#if DYNAMIC_MEM_MELLOC
    fsrc = (FIL *)EXFUNS_MELLOC(sizeof(FIL));    /* 申请内存 */
    fdst = (FIL *)EXFUNS_MELLOC(sizeof(FIL));
    fbuf = (uint8_t *)EXFUNS_MELLOC(FILE_CAT_SIZE);
#else
	FIL ffsrc = {0};
	FIL ffdst = {0};
	uint8_t cpyBuf[FILE_CAT_SIZE] = {0};
	fsrc = &ffsrc;
	fdst = &ffdst;
	fbuf = cpyBuf;
#endif
    ...
    else
    {
        ...
        if (res == 0)           /* 两个都打开成功了 */
        {
            ...
            while (res == 0)    /* 开始复制 */
            {
                res = f_read(fsrc, fbuf, FILE_CAT_SIZE, (UINT *)&br);    /* 源头读出512字节 */
 ...
 */
#if !DYNAMIC_MEM_MELLOC
static uint8_t folderSizeBufScr[MAX_PATHNAME_DEPTH] = {0};
#endif
uint32_t exfuns_get_folder_size(uint8_t *fdname)
{
...
#if DYNAMIC_MEM_MELLOC
    fddir = (DIR *)EXFUNS_MELLOC(sizeof(DIR));   /* 申请内存 */
    finfo = (FILINFO *)EXFUNS_MELLOC(sizeof(FILINFO));
#else
	DIR ffddir = {0};
	FILINFO ffinfo = {0};
	fddir = &ffddir;
    ...
    {
#if DYNAMIC_MEM_MELLOC
        pathname = EXFUNS_MELLOC(MAX_PATHNAME_DEPTH);
#else
		memset(folderSizeBufScr, 0, sizeof(folderSizeBufScr));
		pathname = folderSizeBufScr;
#endif
...
#if !DYNAMIC_MEM_MELLOC
static uint8_t folderNameBufScr[MAX_PATHNAME_DEPTH] = {0};
static uint8_t folderNameBufDst[MAX_PATHNAME_DEPTH] = {0};
#endif
                           ...
                           uint32_t *totsize, uint32_t *cpdsize, uint8_t fwmode)
{
...
#if DYNAMIC_MEM_MELLOC
    srcdir = (DIR *)EXFUNS_MELLOC(sizeof(DIR));  /* 申请内存 */
    dstdir = (DIR *)EXFUNS_MELLOC(sizeof(DIR));
    finfo = (FILINFO *)EXFUNS_MELLOC(sizeof(FILINFO));
#else
	DIR ssrcdir = {0};
	DIR ddstdir = {0};
    ...
    {
#if DYNAMIC_MEM_MELLOC
        dstpathname = EXFUNS_MELLOC(MAX_PATHNAME_DEPTH);
        srcpathname = EXFUNS_MELLOC(MAX_PATHNAME_DEPTH);
#else
		memset(folderNameBufDst, 0, sizeof(folderNameBufDst));
		memset(folderNameBufScr, 0, sizeof(folderNameBufScr));
		dstpathname = folderNameBufDst;
		srcpathname = folderNameBufScr;
#endif
...
//                    dstpathname[dstpathlen + 1] = 0;    /* 结束符 */
                ...
                fcpymsg(dstpathname, 0, 0X04);   /* 更新文件夹名 */
                ...
                while (res == 0)   /* 开始复制文件夹里面的东东 */
                {
                    ...
                    if (finfo->fattrib & 0X10)  /* 是子目录(文件属性,0X20,归档文件;0X10,子目录;) */
                    {
#if DYNAMIC_MEM_MELLOC	/* 静态不支持递归文件夹复制 */
...
#if DYNAMIC_MEM_MELLOC
            EXFUNS_FREE(dstpathname);
            EXFUNS_FREE(srcpathname);
#endif
        }
    }
