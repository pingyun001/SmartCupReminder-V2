...
/**
  ...
  * @retval status
  */
extern int8_t CDC_Control_FS(uint8_t cmd, uint8_t* pbuf, uint16_t length);
...
//        ((USBD_CDC_ItfTypeDef *)pdev->pUserData[pdev->classId])->Control(req->bRequest,
//                                                                         (uint8_t *)req, 0U);
				// 关键修改：对于SET_CONTROL_LINE_STATE，传递wValue
        if (req->bRequest == CDC_SET_CONTROL_LINE_STATE) 
        {
          uint16_t control_state = req->wValue;
          CDC_Control_FS(req->bRequest, (uint8_t *)&control_state, sizeof(control_state));
        }
        else 
        {
